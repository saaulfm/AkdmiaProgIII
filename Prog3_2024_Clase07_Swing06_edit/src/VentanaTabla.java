...
public class VentanaTabla extends JFrame {
	...
	public VentanaTabla(JFrame vAnterior) {
		...
		txtTextoABuscar = new JTextField(20);
		ImageIcon imagenBoton = new ImageIcon("img/lupa.jpg");
		btnBuscar = new JButton(imagenBoton);
		
		pNorte.add(cbTitulos);		
		pNorte.add(txtTextoABuscar);		
		pNorte.add(btnBuscar);	
		...
		btnBuscar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String titulo = cbTitulos.getSelectedItem().toString();
				if(titulo != null) {
					if(titulo.equals("CÓDIGO")) {
						...
						for(int i = 0; i<modeloCompleto.getRowCount(); i++) {
							...
							String nombre = modeloCompleto.getValueAt(i, 1).toString();
							int numCreditos = Integer.parseInt(modeloCompleto.getValueAt(i, 2).toString());
							if(numCreditos == Integer.parseInt(texto)) {
								// Añadir esta fila de la tabla al modelo que va a tener la información filtrada
								Object[] fila = {codigo, nombre, numCreditos};
								modeloFiltrado.addRow(fila);
							}
