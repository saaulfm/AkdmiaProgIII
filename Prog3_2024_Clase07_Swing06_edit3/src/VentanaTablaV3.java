...
public class VentanaTablaV3 extends JFrame {
	...
	private JSlider slCreditos;
	// Declaramos los atributos fila y columna
	private int fila, columna;
	
	public VentanaTablaV3(JFrame vAnterior) {
		// Primera secuencia
		super();
		...
		lblCreditos = new JLabel("Selecciona el número de créditos: ");
		slCreditos = new JSlider(1, 10, 3); // (Valor mínimo, valor máximo, valor inicial)
		slCreditos.setPaintTicks(true); // Para que el usuario pueda ver divisiones entre los valores (con pequeñas líneas) 
		slCreditos.setPaintLabels(true); // Para que el usuario vea los valores numéricos disponibles
		slCreditos.setMinorTickSpacing(1); // Define el espaciado entre las marcas menores (ticks)
		slCreditos.setMajorTickSpacing(1); // Define el espaciado entre las marcas mayores (ticks)
		
		pNorte.add(lblCreditos);
		pNorte.add(slCreditos);
		// Listener
		slCreditos.addChangeListener(new ChangeListener() {
			
			@Override
			public void stateChanged(ChangeEvent e) {
				// Obtener el valor seleccionado en el JSlider
				int valor = slCreditos.getValue();
				// Obtener la información del modelo de la tabla
				DefaultTableModel modeloCompleto = modeloTabla;
				// Crear un nuevo modelo para guardar la información 
				// que coincide con el criterio de búsqueda
				DefaultTableModel modeloFiltrado = new DefaultTableModel();
				modeloFiltrado.setColumnIdentifiers(titulos);
				// Recorrer el modelo que tiene toda la información de la tabla
				for(int i = 0; i<modeloCompleto.getRowCount(); i++) {
					// Por cada  fila, obtener toda su información
					String codigo = modeloCompleto.getValueAt(i, 0).toString();
					String nombre = modeloCompleto.getValueAt(i, 1).toString();
					int numCreditos = Integer.parseInt(modeloCompleto.getValueAt(i, 2).toString());
					if(numCreditos >= valor) {
						// Añadir esta fila de la tabla al modelo que va a tener la información filtrada
						Object [] fila = {codigo, nombre, numCreditos};
